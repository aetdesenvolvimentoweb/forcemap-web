<script lang="ts">
  ...
  let usedMilitaryIds = $derived(new Set(officers.map((o) => o.military.id)));
  ...
  let editAvailableMilitary = $derived(
    selected
      ? [
          ...availableMilitary,
          ...(selected.military.militaryRank.order <= OFFICER_RANK_ORDER_LIMIT
            ? [selected.military]
            : []),
        ].sort((a, b) => a.militaryRank.order - b.militaryRank.order)
      : availableMilitary,
    ...
    {#snippet rows()}
      {#each sorting.sorted as officer}
        <tr>
          ...
          <td>{formatWorkPeriod(officer.workPeriod)}</td>
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col">
              ...
              <span class="text-xs text-base-content/60"
                >RG {officer.military.rg} · {formatWorkPeriod(officer.workPeriod)} · {officer.workSchedule}</span
              >
            </div>
          </div>
