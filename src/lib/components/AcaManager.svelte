<script lang="ts">
  ...
  let usedMilitaryIds = $derived(new Set(acas.map((a) => a.military.id)));
...
<div class="flex flex-col gap-4">
  <PageToolbar title="ACA" newLabel="Novo ACA" onNew={openCreate} bind:search />
    ...
    {#snippet rows()}
      {#each sorting.sorted as aca}
        <tr>
          ...
          <td>{formatWorkPeriod(aca.workPeriod)}</td>
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col">
              ...
              <span class="text-xs text-base-content/60"
                >RG {aca.military.rg} · {formatWorkPeriod(aca.workPeriod)} · {aca.workSchedule}</span
              >
            </div>
          </div>
